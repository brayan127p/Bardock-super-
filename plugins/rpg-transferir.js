...
`.trim();
  ...
  if (!(who in global.db.data.users)) return conn.sendMessage(m.chat, {text: `*🏴‍☠️ 𝑬𝒍 𝒖𝒔𝒖𝒂𝒓𝒊𝒐 ${who} 𝒏𝒐 𝒆𝒔𝒕𝒂 𝒆𝒏 𝒍𝒂 𝒃𝒂𝒔𝒆 𝒅𝒆 𝒅𝒂𝒕𝒐𝒔.*`, mentions: [m.sender]}, {quoted: m});
...
*◉ no = 𝒑𝒂𝒓𝒂 𝒄𝒂𝒏𝒄𝒆𝒍𝒂𝒓*`.trim();
  ...
  confirmation[m.sender] = { sender: m.sender, to: who, message: m, type, count, timeout: setTimeout(() => (conn.sendMessage(m.chat, {text: '*☠️ 𝑺𝒆 𝒂𝒄𝒂𝒃𝒐 𝒆𝒍 𝒕𝒊𝒆𝒎𝒑𝒐, 𝒏𝒐 𝒔𝒆 𝒐𝒃𝒕𝒖𝒗𝒐 𝒓𝒆𝒔𝒑𝒖𝒆𝒔𝒕𝒂. 𝑻𝒓𝒂𝒏𝒔𝒇𝒆𝒓𝒆𝒏𝒄𝒊𝒂 𝒄𝒂𝒏𝒄𝒆𝒍𝒂𝒅𝒂.*', mentions: [m.sender]}, {quoted: m}), delete confirmation[m.sender]), 60 * 1000)};
}
...
handler.before = async (m) => {
  ...
  if (/^No|no$/i.test(m.text)) {
    ...
    return conn.sendMessage(m.chat, {text: '*🏴‍☠️ 𝑪𝒂𝒏𝒄𝒆𝒍𝒂𝒅𝒐, 𝒍𝒂 𝒕𝒓𝒂𝒏𝒔𝒇𝒆𝒓𝒆𝒏𝒄𝒊𝒂 𝒏𝒐 𝒔𝒆 𝒓𝒆𝒂𝒍𝒊𝒛𝒂𝒓𝒂.*', mentions: [m.sender]}, {quoted: m});
  }
